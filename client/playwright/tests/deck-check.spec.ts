...
test.describe('Deck Check Page', () => {
  ...
  test('DC-01: Should display 3 counters (Creatures, Trainer, Energy)', async ({ page }) => {
    await expect(page.getByText(/creatures/i)).toBeVisible();
  ...
  test('DC-02: Each counter should display current value (initially 0)', async ({ page }) => {
    ...
    await expect(creaturesSection.getByText('0').first()).toBeVisible();
  ...
  test('DC-03: Each counter should have increment buttons +1, +2, +3, +4', async ({ page }) => {
    ...
    const buttons = creaturesSection.getByRole('button');
  ...
  test('DC-04: Clicking increment button should update the counter', async ({ page }) => {
    ...
    await creaturesSection.getByRole('button').nth(1).click(); // +2 button
    await expect(creaturesSection.getByText('2').first()).toBeVisible();
    
    await creaturesSection.getByRole('button').nth(2).click(); // +3 button
    await expect(creaturesSection.getByText('5').first()).toBeVisible();
  });
  ...
  test('DC-05: Total should update when counters change', async ({ page }) => {
    ...
    await creaturesSection.getByRole('button').nth(3).click(); // +4 button
  ...
  test('DC-06: Should have UNDO button that reverts last action', async ({ page }) => {
    ...
    await creaturesSection.getByRole('button').nth(2).click(); // +3 button
    await expect(creaturesSection.getByText('3').first()).toBeVisible();
  ...
  test('DC-09: Should persist counters in localStorage', async ({ page }) => {
    ...
    const creaturesAfterReload = page.locator('div', { has: page.getByText(/^creatures$/i) }).first();
    const trainerAfterReload = page.locator('div', { has: page.getByText(/^trainer$/i) }).first();
    
    await expect(creaturesAfterReload.getByText('4').first()).toBeVisible();
  ...
  test('Mobile-first: Should be usable on 375px viewport', async ({ page }) => {
    ...
    const firstButton = creaturesSection.getByRole('button').nth(0);
  ...
  test('Counter selection: Should highlight selected counter', async ({ page }) => {
    // Find the Paper containing "Creatures" and click one of its buttons
    await page.locator('[class*="mantine-Paper"]', { has: page.getByText(/^creatures$/i) })
    ...
    const paperElement = page.locator('[class*="mantine-Paper"][data-selected="true"]', { 
      has: page.getByText(/^creatures$/i) 
    });
  ...
  test('Undo multiple times should work correctly', async ({ page }) => {
    ...
    await creaturesSection.getByRole('button').nth(1).click(); // +2 button
    await trainerSection.getByRole('button').nth(2).click(); // +3 button
    await creaturesSection.getByRole('button').nth(0).click(); // +1 button
    ...
    await undoButton.click();
    const creaturesAfterUndo = page.locator('div', { has: page.getByText(/^creatures$/i) }).first();
    await expect(creaturesAfterUndo.getByText('0').first()).toBeVisible();
  });
  ...
  test('Total should turn green when count reaches exactly 60', async ({ page }) => {
    ...
    for (let i = 0; i < 15; i++) {
      await creaturesSection.getByRole('button').nth(3).click(); // +4 x 15 = 60 total
    }
  ...
  test('Total should turn red when count exceeds 60', async ({ page }) => {
    ...
    for (let i = 0; i < 16; i++) {
      await creaturesSection.getByRole('button').nth(3).click(); // +4 x 16 = 64 total
    }
